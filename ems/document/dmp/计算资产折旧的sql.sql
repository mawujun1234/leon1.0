...
--插入初始化数据
insert into ems_assetclean(id,ecode,day_now,day_have)
...
create index AssetClean_ecode on EMS_ASSETCLEAN (ecode);
create index AssetClean_day_now on EMS_ASSETCLEAN (day_now);
-- Create/Recreate indexes 
create index lifecycle on EMS_EQUIPMENTCYCLE (ecode);
--初始化所有设备，默认使用年限是5年
update ems_equipmentprod set depreci_year=5 where depreci_year is null;
--计算老旧订单的剩余使用年限
update ems_assetclean a set day_have=(
...
--更新原值
update ems_assetclean a set value_original=(
...
--计算净值和原值
update ems_assetclean a set value_depreciation=(value_original*0.95/day_have*day_used);
update ems_assetclean a set value_net=(value_original-value_depreciation);

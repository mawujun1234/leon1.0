...
public class CopyOfSpringMVCControllerTest {
	@Autowired
	...
	@Test
	public void queryPageAutoWarp() throws Exception {
		this.mockMvc.perform(get("/test/queryPageAutoWarp.do").accept(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())
		.andExpect(content().contentType("application/json"))
		.andExpect(jsonPath("$.success").value(true))
		.andExpect(jsonPath("$.total").value(100))
